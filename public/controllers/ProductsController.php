...
class ProductController {
    ...
    public function actualizarCantidad() {
        ...
        if (isset($_GET['id']) && isset($_GET['operacion'])) {
            $producto_id = $_GET['id'];
            ...
            if (isset($_SESSION['carrito'][$producto_id])) {
                ...
                } elseif ($operacion === 'disminuir' && $_SESSION['carrito'][$producto_id] > 1) {
                    $_SESSION['carrito'][$producto_id]--;
                }
            }
        }
        
        // Redirigir de vuelta al carrito sin ancla
    ...
    public function confirmarPedido() {
        ...
        foreach ($_SESSION['carrito'] as $producto_id => $cantidad) {
            $verificacion = $pDAO->verificarStock($producto_id, $cantidad);
            if (!$verificacion['disponible']) {
                $_SESSION['mensaje'] = "Error: " . $verificacion['mensaje'];
                header("Location: index.php?controller=ProductController&action=verCarrito");
                exit();
            }
        }
        ...
        if ($pedido_id) {
            // Agregar productos al pedido Y REDUCIR STOCK
            foreach ($productosCarrito as $producto) {
                ...
                $pDAO->reducirStock($producto['id'], $producto['cantidad']);
            }
