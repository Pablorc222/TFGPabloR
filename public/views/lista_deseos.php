...
<head>
    ...
    <title>Lista de Deseos - SoccerStore</title>
</head>
<body>
    <div style="font-family: Arial, sans-serif; display: flex; flex-direction: column; align-items: center; padding: 60px;">
        ...
        <div style="width: 50%; background-color: #fff; border-radius: 10px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); padding: 20px; margin-bottom: 20px; text-align: center;">
            ...
            <?php else: ?>
                <?php foreach ($listaDeseos as $producto): ?>
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px solid #eee; text-align: left;">
                        <div>
                            ...
                            <p style="margin: 5px 0;">Precio: <?php echo htmlspecialchars($producto['Precio']); ?>€</p>
                        </div>
                        <div style="display: flex; gap: 10px;">
                            <a href="index.php?controller=ProductController&action=verDetallesProducto&idProducto=<?php echo htmlspecialchars($producto['ID_Producto']); ?>" 
                               style="text-decoration: none; color: inherit; padding: 10px; background-color: #437F97; color: white; border: none; border-radius: 5px; cursor: pointer;">
                                Ver
                            </a>
                            <a href="index.php?controller=ProductController&action=addCarrito&id=<?php echo htmlspecialchars($producto['ID_Producto']); ?>&cantidad=1" 
                               style="text-decoration: none; color: inherit; padding: 10px; background-color: #28a745; color: white; border: none; border-radius: 5px; cursor: pointer;">
                                Añadir al carrito
                            </a>
                            <a href="index.php?controller=ListaDeseosController&action=eliminarProducto&producto_id=<?php echo htmlspecialchars($producto['ID_Producto']); ?>&redirect=lista" 
                               style="text-decoration: none; color: inherit; padding: 10px; background-color: #dc3545; color: white; border: none; border-radius: 5px; cursor: pointer;">
                                Eliminar
                            </a>
                        </div>
                    </div>
